...
const Hyperspeed = ({ effectOptions = DEFAULT_EFFECT_OPTIONS, direction = 'forward' }) => {
  ...
  useEffect(() => {
    ...
    class App {
      constructor(container, options = {}) {
        ...
        this.camera.position.y = options.cameraY !== undefined ? options.cameraY : 8;
        this.camera.position.x = 0;
        this.scene = new THREE.Scene();
        this.scene.background = null;

        let fog = new THREE.Fog(options.colors.background, options.length * 0.2, options.length);
        this.scene.fog = fog;
        this.fogUniforms = {
      ...
      update(delta) {
        // Tuned to 0.05 so it visibly accelerates from the slow starting speed within the 1 second jump
        let lerpPercentage = Math.exp(-(-60 * Math.log2(1 - 0.05)) * delta);
    ...
    myApp.loadAssets().then(myApp.init).then(() => {
      // Trigger the warp acceleration automatically!
      if (appRef.current) {
        appRef.current.fovTarget = options.fovSpeedUp || 140;
        appRef.current.speedUpTarget = options.speedUp || 8;
      }
    });
